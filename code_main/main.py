...
def main(argv):
    ...
    ap.add_argument("-cp", "--cam_pair", action="store_true", default=False, help="generate camera pair data file")
...
def transition_test_main(cfg, wr=0):
    if wr:
        transition_time_write(cfg)
    transition_time_plot(cfg)


def transition_time_plot(cfg):
    ts_str = []
    # March 5th, 2019
    time_year = 2019
    time_mon = 3
    ...
    for time_mday in range(5, 6):
        for time_hour in range(6, 18):     # Every hour
            for time_min in range(0, 60, 15):  # Every 15 minutes
                ts_str.append("./tmp/dir_ts_{d}_{m}_{y}_{h}_{min}.json".format(d=time_mday, m=time_mon, y=time_year, h=time_hour, min=time_min))

    travel_data = dict()
    traffic_data = dict()

    for ts in ts_str:
        dir_dat = json_read(ts)
        # dir_dat = dat['direction']

        for k in dir_dat:
            travel_time = dir_dat[k][0]['legs'][0]['duration']['value']
            traffic_time = dir_dat[k][0]['legs'][0]['duration_in_traffic']['value']

            if k not in travel_data:
                travel_data[k] = [travel_time]
                traffic_data[k] = [traffic_time]
            else:
                travel_data[k].append(travel_time)
                traffic_data[k].append(traffic_time)

    klist = list(dir_dat.keys())
    rkeys = [klist[k] for k in random.sample(range(0, 116), 10)]

    plt.figure(0)
    for k in rkeys:
        plt.plot(travel_data[k])
    # plt.plot()
    # plt.show()

    plt.figure(1)
    for k in rkeys:
        plt.plot(traffic_data[k])
    # plt.plot()
    plt.show()
    print('done')

def transition_time_write(cfg):
    lmpairs = cfg['pair'][20, 34, 41, 57, 60, 88, 100, 101, 104, 110]
    wplist = cfg['gps']
    ...
    for time_mday in range(5, 20):
        for time_hour in range(6, 18):     # Every hour
            for time_min in range(0, 60, 15):  # Every 15 minutes
                ts_str.append("{d}_{m}_{y}_{h}_{min}".format(d=time_mday, m=time_mon, y=time_year, h=time_hour, min=time_min))
                time_strt = time.strptime(ts_str[-1],
                    "%d_%m_%Y_%H_%M")
                timestamps.append(time.mktime(time_strt))
    
    for ts, tstr in zip(timestamps, ts_str):
        json = gmaps_dir_json(lmpairs, wplist, 'str', ts)
        json_write('./tmp/dir_ts_{0}'.format(tstr), json)
...
def get_camera_pairs(setA, setB):
    ...
    if not (setA - setB):  # setA is the same as setB
        listA = list(setA)
        for i in range(0, len(listA)):
            for j in range(i+1, len(listA)):
                cam_overlap.append((listA[i], listA[j]))
        return [], set(cam_overlap)
    elif not (setA & setB):  # No points in common
        for cA in setA_u:
            for cB in setB_u:
                cam_pairs.append((cA, cB))
        return set(cam_pairs), []
    else:
        for cA in set_intersect:
            for cB in (setB_u | setA_u):
                cam_overlap.append((cA, cB))
        for cA in setA_u:
            for cB in setB_u:
                cam_pairs.append((cA, cB))
        return set(cam_pairs), set(cam_overlap)
...
def gmaps_dir_extract(gps, departure_time="now"):
...
def gmaps_dir_json(lmpairs, wplist, ktype='int', dtime='now'):
    res = dict()
    ...
    for o, d in lmpairs:
        ...
        if ktype == 'str':
            o = str(o)
            d = str(d)
        _, _, _, query = gmaps_dir_extract([wplist[o], wplist[d]], dtime)
