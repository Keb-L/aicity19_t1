...
def bg_subtract(arg_in):
    ...
    for v in vpath:  # Test Videos only, modify this to include other videos
        ...
        learn_rate = arg_in.learn_rate                                       # Initial Learning rate
        ...
        while True:
            ...
            if arg_in.play:
                ...
                if kp == ord('s'):
                    ...
                    # cv2.imwrite(dirname + "bg_img_{0}_{1}_{2}.jpg".format(*v.split('/')[-1:], str(uuid.uuid4())[0:10]), bg_img)
                elif kp == ord('n'):
                    break
                elif kp == ord('q'):
        ...
        if arg_in.image:
            bg_img = bs.getBackgroundImage()
            # cv2.imwrite(dirname + "bg_img_{0}_.jpg".format(*v.split('/')[-1:]), bg_img)
